...
2. Extract :file:`IntraMaps Roam Admin.zip` into :file:`C:\\IntraMaps Roam Admin`
...
3. Run :file:`Roam Admin.bat` from :file:`C:\\IntraMaps Roam Admin`
...
Before we open Roam on the device we need to populate the database with the data from the server.  
...
1. Run :file:`C:\\IntraMaps Roam\\projects\\melton_firebreak\\Sync-Inspections.bat`

The first sync will take longer than others than any further syncs for that client. Specifically due to having to replicate the Inspection and Photo table.
